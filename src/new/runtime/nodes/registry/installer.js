...
module.exports = class Installer {
    ...
    paletteEditorEnabled() {
        return this.paletteEditorEnabled;
    }


    checkModulePath(folder) {
        var moduleName;
        var err;
    ...
    checkExistingModule(module, version) {
    ...
    installModule(module, version) {
        ...
        return when.promise(function (resolve, reject) {
            ...
            if (!isUpgrade) {
                log.info(log._('server.install.installing', {
                    name: module,
                    version: version || 'latest'
                }));
            } else {
                log.info(log._('server.install.upgrading', {
                    name: module,
                    version: version || 'latest'
                }));
            }
                ...
                function (err, stdin, stdout) {
                    if (err) {
                        ...
                        if (lookFor404.test(stdout)) {
                            log.warn(log._('server.install.install-failed-not-found', {
                                name: module
                            }));
                            ...
                            log.warn(log._('server.install.upgrade-failed-not-found', {
                                name: module
                            }));
                            e = new Error('Module not found');
                            e.code = 404;
                            ...
                            log.warn(log._('server.install.install-failed-long', {
                                name: module
                            }));
                            log.warn('------------------------------------------');
                        ...
                        if (!isUpgrade) {
                            log.info(log._('server.install.installed', {
                                name: module
                            }));
                            resolve(require('./index').addModule(module).then(reportAddedModules));
                        } else {
                            log.info(log._('server.install.upgraded', {
                                name: module,
                                version: version
                            }));
                            events.emit('runtime-event', {
                                ...
                                payload: {
                                    type: 'warning',
                                    text: 'notification.warnings.restartRequired'
                                },
                                retain: true
                            });
    ...
    reportRemovedModules(removedNodes) {
    ...
    uninstallModule(module) {
        return when.promise(function (resolve, reject) {
            ...
            try {
                ...
                return reject(new Error(log._('server.install.uninstall-failed', {
                    name: module
                })));
            }

            var list = registry.removeModule(module);
            log.info(log._('server.install.uninstalling', {
                name: module
            }));
            var child = child_process.execFile(npmCommand, ['remove', '--save', module], {
                ...
                function (err, stdin, stdout) {
                    if (err) {
                        log.warn(log._('server.install.uninstall-failed-long', {
                            name: module
                        }));
                        log.warn('------------------------------------------');
                        log.warn(err.toString());
                        log.warn('------------------------------------------');
                        reject(new Error(log._('server.install.uninstall-failed', {
                            name: module
                        })));
                    } else {
                        log.info(log._('server.install.uninstalled', {
                            name: module
                        }));
